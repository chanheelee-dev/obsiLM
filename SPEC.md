...
| 상황 | 처리 방식 |
|---|---|
| URL 접근 불가 | 에러 메시지 출력 후 중단 |
| PDF 파싱 실패 | 텍스트 추출 fallback (pypdf) |
| YouTube 자막 없음 | 에러 메시지 + 수동 추가 안내 |
| 파일 미존재 | 명확한 에러 메시지 출력 |

### 8.2 AI API 에러

- **Rate limit**: 자동 재시도 (지수 백오프, 최대 3회)
- **컨텍스트 초과**: Phase 2에서 청킹 + RAG로 처리 (MVP는 에러 안내)
- **API 키 미설정**: 실행 시점에 바로 안내 메시지 출력

### 8.3 vault 무결성

- 명령 실행 중 인터럽트 발생 시 부분 생성된 파일 자동 정리
- `.obsilm` 파일 손상 시 복구 안내 메시지 출력
- `notebook.md` 없을 경우 자동 재생성
